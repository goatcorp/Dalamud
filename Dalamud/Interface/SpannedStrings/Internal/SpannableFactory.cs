...
internal class SpannableFactory : IInternalDisposableService, ISpannableFactory
{
    ...
    private readonly SpannedStringRenderer?[] rendererPool;
    ...
    {
        this.rendererPool = new SpannedStringRenderer?[8];
    ...
    /// <inheritdoc/>
    public unsafe ISpannedStringRenderer Rent(
        ISpannedStringRenderer.Usage usage, ISpannedStringRenderer.Options options = default)
    {
        ...
        var instance = default(SpannedStringRenderer);
    ...
    /// <summary>Returns the finished instance of <see cref="SpannedStringRenderer"/>.</summary>
    /// <param name="renderer">The instance to return.</param>
    /// <remarks>For use with <see cref="SpannedStringRenderer"/>.</remarks>
    internal void Return(SpannedStringRenderer? renderer)
    {
        if (renderer is null)
            return;
        if (!this.disposing)
