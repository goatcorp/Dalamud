...
public sealed class UiBuilder : IDisposable
{
    ...
    /// </summary>
    /// <value>An IUnknown that points to a Direct3D device, or <c>0</c> if not (yet) available.</value>
    public nint DevicePtr => this.InterfaceManagerWithScene?.Device ?? nint.Zero;

    /// <summary>
    /// Gets the game's main window handle.
    /// </summary>
    /// <value>A HWND that points to the main window, or <c>0</c> if not (yet) available.</value>
    public nint WindowHandlePtr => this.InterfaceManagerWithScene?.WindowHandlePtr ?? nint.Zero;
