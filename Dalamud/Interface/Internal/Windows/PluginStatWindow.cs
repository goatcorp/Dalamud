            ...
            if (ImGui.BeginTabItem("Draw times"))
            {
                ...
                if (doStats)
                {
                    ...
                    if (ImGui.Button("Reset"))
                    {
                        ...
                        {
                            plugin.PluginInterface.UiBuilder.LastDrawTime = -1;
                            plugin.PluginInterface.UiBuilder.MaxDrawTime = -1;
                            plugin.PluginInterface.UiBuilder.DrawTimeHistory.Clear();
                        }
                    }
                    ...
                    foreach (var plugin in this.pluginManager.Plugins)
                    {
                        ImGui.Text(plugin.Definition.Name);
                        ImGui.NextColumn();

                        ImGui.Text($"{plugin.PluginInterface.UiBuilder.LastDrawTime / 10000f:F4}ms");
                        ImGui.NextColumn();

                        ImGui.Text($"{plugin.PluginInterface.UiBuilder.MaxDrawTime / 10000f:F4}ms");
                        ImGui.NextColumn();

                        if (plugin.PluginInterface.UiBuilder.DrawTimeHistory.Count > 0)
                        {
                            ImGui.Text($"{plugin.PluginInterface.UiBuilder.DrawTimeHistory.Average() / 10000f:F4}ms");
                        }
                        else
                        {
                            ImGui.Text("-");
