...
internal class TextSpannableWidget : IDataWindowWidget, IDisposable
{
    ...
    private void SetupControls()
    {
            ...
            Children =
            {
                ...
                new LinearContainer
                {
                    ...
                    Children =
                    {
                                        ...
                                        new RadioControl
                                            {
                                                ...
                                                IconSide = BooleanControl.Side.Top,
                        ...
                        new LabelControl
                        {
                            Name = "lblLinearContainerTestsAlignmentH",
                            Text = "Horizontal Alignment: 0.00",
                            Margin = new(0, 4),
                            TextStyle = h2Style,
                        }.GetAsOut(out var lblLinearContainerTestsAlignmentH),
                        new ScrollBarControl
                        {
                            Name = "scrLinearContainerTestsAlignmentH",
                            Size = new(ControlSpannable.MatchParent, 48),
                            MinValue = 0f,
                            MaxValue = 1f,
                            AlignValue = 0.05f,
                            DecreaseButton = null,
                            IncreaseButton = null,
                        }.GetAsOut(out var scrLinearContainerTestsAlignmentH),
                        new ControlSpannable { Size = new(0, 12) },
                        new LabelControl
                        {
                            Name = "lblLinearContainerTestsAlignmentV",
                            Text = "Vertical Alignment: 0.00",
                            Margin = new(0, 4),
                            TextStyle = h2Style,
                        }.GetAsOut(out var lblLinearContainerTestsAlignmentV),
                        new ScrollBarControl
                        {
                            Name = "scrLinearContainerTestsAlignmentV",
                            Size = new(ControlSpannable.MatchParent, 48),
                            MinValue = 0f,
                            MaxValue = 1f,
                            AlignValue = 0.05f,
                            DecreaseButton = null,
                            IncreaseButton = null,
                        }.GetAsOut(out var scrLinearContainerTestsAlignmentV),
                        new ControlSpannable { Size = new(0, 12) },
                        new LabelControl
                        {
                ...
                new LinearContainer
                {
                    ...
                    Children =
                    {
                        ...
                        {
                            Name = "lblNotALink",
                            SpannableText = new StyledTextBuilder().Append("Not a Link").CreateSpannable(),
        ...
        pzt.ViewportChanged += () =>
        {
            ...
            this.rootContainer.Scroll =
                ...
                this.rootContainer.MeasuredContentBox.Center - pzt.Pan;
            if (pzt.EffectiveZoom > 0f)
                linearContainer.Scale = pzt.EffectiveZoom;
            linearContainer.Transformation = Matrix4x4.CreateRotationZ(pzt.Rotation);
        };
        this.rootContainer.MeasuredBoundaryBoxChange += _ => { pzt.Size = linearContainer.MeasuredBoundaryBox.Size; };
        ...
        {
            foreach (var x in linearContainer.Children.OfType<LinearContainer>())
            {
                x.Direction =
                    e.NewValue
        ...
        scrLinearContainerTestsAlignmentH.Scroll += e =>
        {
            lblLinearContainerTestsAlignmentH.Text = $"Horizontal Alignment: {e.NewValue:0.00}";
            foreach (var x in linearContainer.EnumerateHierarchy<LinearContainer>())
            {
                    ...
                    or LinearDirection.BottomToTop)
                {
                    for (var i = 0; i < x.Children.Count; i++)
                        x.SetChildLayout(i, x.GetChildLayout(i) with { Alignment = e.NewValue });
                }
            }
        };
        scrLinearContainerTestsAlignmentV.Scroll += e =>
        {
            lblLinearContainerTestsAlignmentV.Text = $"Horizontal Alignment: {e.NewValue:0.00}";
            foreach (var x in linearContainer.EnumerateHierarchy<LinearContainer>())
            {
        ...
        void UpdateLblOptions(PropertyChangeEventArgs<bool>? e)
        {
            ...
            lblOptions.TextStyle = TextStyle.FromContext;
            lblOptions.SpannableText =
                this.UpdateTextSpannableConfig(
    ...
    private void DrawTestComplicatedTextBlock(StyledTextBuilder ssb, float x)
    {
        if (this.lblComplicated is null)
        {
            ...
            {
                SpannableText = this.UpdateTextSpannableConfig(ssb.Build().CreateSpannable()),
                Size = new(ControlSpannable.MatchParent, ControlSpannable.WrapContent),
            };
    ...
    private void DrawDynamicOffsetTest(StyledTextBuilder ssb)
    {
        ...
        var ts = this.UpdateTextSpannableConfig(ssb.CreateSpannable());
        ts.VerticalAlignment = 0.5f;
        ts.Style = TextStyle.FromContext with { HorizontalAlignment = 0.5f };
        Service<SpannableRenderer>.Get().Draw(
            ts,
            new(
    ...
    private void DrawTransformationTest(StyledTextBuilder ssb)
    {
        ...
        var ts = this.UpdateTextSpannableConfig(ssb.CreateSpannable());
        ts.WordBreak = WordBreakType.KeepAll;
        ts.Style = TextStyle.FromContext with
        {
            EdgeWidth = 1f,
