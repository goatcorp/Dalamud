    ...
    public unsafe SelfTestStepResult RunStep()
    {
        ...
        try
        {
            for (var i = 0; i < this.redirects.Length; i++)
            {
                ...
                var rowId1 = (int)redirect.RowId;
                var colIndex1 = (int)ushort.MaxValue;
                var flags1 = RaptureTextModule.Instance()->ResolveSheetRedirect(utf8SheetName, &rowId1, &colIndex1);
                ...
                if (utf8SheetName->ToString() != sheetName2 || rowId1 != rowId2 || colIndex1 != colIndex2 || (int)flags1 != (int)flags2)
                {
