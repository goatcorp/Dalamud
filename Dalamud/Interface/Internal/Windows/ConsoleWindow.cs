...
internal class ConsoleWindow : Window, IDisposable
{
    ...
    private void ProcessCommand()
    {
        try
        {
            ...
            this.lastCmdSuccess = Service<ConsoleManager>.Get().ProcessCommand(
                this.commandText,
                diagnostic =>
                {
                    diagnostic.WriteToLog();
                    Log.Error("Usage: {Usage}", diagnostic.Entry.GetUsageString());
                }) == ConsoleManager.CommandProcessResult.Success;
