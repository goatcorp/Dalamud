...
public sealed partial class StyledTextBuilder
{
    ...
    private delegate StyledTextBuilder AppendSpanFormattableDelegate<in T>(T value, int repeat = 1)
        where T : struct;

    /// <inheritdoc/>
    public StyledTextBuilder Append(ReadOnlySpan<char> span, int repeat = 1)
    {
    ...
    /// <inheritdoc/>
    public StyledTextBuilder Append(ReadOnlySpan<byte> span, int repeat = 1)
    {
        if (repeat < 1 || span.IsEmpty)
            return this;
    ...
    /// <inheritdoc/>
    public StyledTextBuilder Append(ReadOnlyMemory<char> memory, int repeat = 1) =>
        this.Append(memory.Span, repeat);

    /// <inheritdoc/>
    public StyledTextBuilder Append(ReadOnlyMemory<byte> memory, int repeat = 1) =>
        this.Append(memory.Span, repeat);

    /// <inheritdoc/>
    public StyledTextBuilder Append(string? text, int repeat = 1) =>
        this.Append((text ?? "null").AsSpan(), repeat);

    /// <inheritdoc/>
    public StyledTextBuilder Append(UtfEnumerator utfEnumerator, int repeat = 1)
    {
        while (repeat-- > 0)
        {
    ...
    /// <inheritdoc/>
    public StyledTextBuilder Append(ISpanFormattable? value, int repeat = 1) =>
        value is null ? this.Append("null"u8) : this.AppendSpanFormattable(value, repeat);

    /// <inheritdoc/>
    public StyledTextBuilder Append(IUtf8SpanFormattable? value, int repeat = 1) =>
        value is null ? this.Append("null"u8) : this.AppendUtf8SpanFormattable(value, repeat);

    /// <inheritdoc/>
    public StyledTextBuilder Append(object? value, int repeat = 1) =>
        this.Append((value?.ToString() ?? "null").AsSpan(), repeat);

    /// <inheritdoc/>
    public StyledTextBuilder Append(char value, int repeat = 1) => this.AppendChar(value, repeat);

    /// <inheritdoc/>
    public StyledTextBuilder Append<T>(T value, int repeat = 1) where T : struct
    {
    ...
    /// <inheritdoc/>
    public StyledTextBuilder AppendSpannable(ISpannableTemplate? callback, out int id)
    {
    ...
    /// <inheritdoc/>
    public StyledTextBuilder AppendSpannable(int id)
    {
    ...
    /// <inheritdoc/>
    public StyledTextBuilder AppendChar(int codepoint, int repeat = 1)
    {
        if (repeat < 1)
            return this;
    ...
    /// <inheritdoc/>
    public StyledTextBuilder AppendChar(Rune rune, int repeat = 1)
    {
        if (repeat < 1)
            return this;
    ...
    /// <inheritdoc/>
    public StyledTextBuilder AppendIcon(GfdIcon iconId) => this.AppendIcon((int)iconId);

    /// <inheritdoc/>
    public StyledTextBuilder AppendIcon(int iconId)
    {
    ...
    /// <inheritdoc/>
    public StyledTextBuilder AppendTexture(int id) => this.AppendTexture(id, Vector2.Zero, Vector2.One);

    /// <inheritdoc/>
    public StyledTextBuilder AppendTexture(int id, Vector2 uv0, Vector2 uv1)
    {
    ...
    /// <inheritdoc/>
    public StyledTextBuilder AppendTexture(IDalamudTextureWrap? textureWrap, out int id) =>
        this.AppendTexture(textureWrap, Vector2.Zero, Vector2.One, out id);

    /// <inheritdoc/>
    public StyledTextBuilder AppendTexture(IDalamudTextureWrap? textureWrap, Vector2 uv0, Vector2 uv1, out int id)
    {
    ...
    /// <inheritdoc/>
    public StyledTextBuilder AppendLine(NewLineType newLineType = NewLineType.Manual)
    {
        switch (newLineType)
        {
    ...
    /// <inheritdoc/>
    public StyledTextBuilder AppendLine(
    ...
    private StyledTextBuilder AppendSpanFormattable<T>(T value, int repeat = 1) where T : ISpanFormattable
    {
        if (repeat < 1)
            return this;
    ...
    private StyledTextBuilder AppendUtf8SpanFormattable<T>(T value, int repeat = 1) where T : IUtf8SpanFormattable
    {
        if (repeat < 1)
            return this;
