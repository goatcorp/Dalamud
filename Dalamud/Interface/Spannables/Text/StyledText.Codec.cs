...
public sealed partial class StyledText
{
    ...
    public static StyledText Parse(string s, IFormatProvider? provider) =>
        TryParse(s, provider, out var result, out var exception) ? result : throw exception;

    /// <inheritdoc cref="IParsable{TSelf}.TryParse(string?, IFormatProvider?, out TSelf)"/>
    public static bool TryParse(string? s, IFormatProvider? provider, [NotNullWhen(true)] out StyledText? result) =>
        TryParse(s, provider, out result, out _);

    /// <inheritdoc cref="ISpanParsable{TSelf}.Parse(ReadOnlySpan{char}, IFormatProvider?)"/>
    public static StyledText Parse(ReadOnlySpan<char> s, IFormatProvider? provider) =>
        TryParse(s, provider, out var result, out var exception) ? result : throw exception;
    ...
    public static bool TryParse(
        ...
        [NotNullWhen(true)] out StyledText? result) =>
        TryParse(s, provider, out result, out _);
    ...
    public static bool TryDecode(ReadOnlySpan<byte> source, [NotNullWhen(true)] out StyledText? value)
    {
        value = null;
        ...
        value = new(
            ...
            new ISpannableTemplate?[numCallbacks]);
        return true;
    }
        ...
        foreach (var segment in this.AsMemory().AsSpan())
        {
            ...
            else if (segment.TryGetRecord(out var record, out var data))
            {
                    ...
                    && SpannedRecordCodec.TryDecodeFontHandleSetIndex(data, out var setIndex))
                {
                    ...
                    {
                        DalamudDefaultFontAndFamilyId => nameof(IStyledTextBuilder.PushDefaultFontFamily),
                        DalamudAssetFontAndFamilyId => nameof(IStyledTextBuilder.PushAssetFontFamily),
                        GameFontAndFamilyId => nameof(IStyledTextBuilder.PushGameFontFamily),
                        SystemFontFamilyId => nameof(IStyledTextBuilder.PushSystemFontFamilyIfAvailable),
                        not null => nameof(IStyledTextBuilder.PushFontFamily),
                        _ => nameof(IStyledTextBuilder.PushFontSet),
                    };
        ...
        [NotNullWhen(false)] out Exception? exception)
    {
        ...
        while (!s.IsEmpty)
        {
            ...
            foreach (var method in SsbMethods)
            {
                ...
                var allArgsSpan = s;
                if (method.Info.Name == nameof(StyledTextBuilder.PushLink))
                {
