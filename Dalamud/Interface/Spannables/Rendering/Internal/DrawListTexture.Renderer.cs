...
internal sealed unsafe partial class DrawListTexture
{
    ...
    internal sealed class Renderer : IInternalDisposableService
    {
        ...
        public void MakeStraight(ID3D11UnorderedAccessView* puav, RectVector4 clipRect)
        {
            ThreadSafety.AssertMainThread();
            ...
            var scissorRect = new RECT(0, 0, (int)clipRect.Width, (int)clipRect.Height);
            this.deviceContext.Get()->RSSetScissorRects(1, &scissorRect);
            this.deviceContext.Get()->RSSetState(this.rasterizerState);
            var viewport = new D3D11_VIEWPORT(0, 0, clipRect.Width, clipRect.Height);
            this.deviceContext.Get()->RSSetViewports(1, &viewport);
