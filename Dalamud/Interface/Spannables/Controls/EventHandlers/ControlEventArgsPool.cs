...
public static class ControlEventArgsPool
{
    ...
    public static T Rent<T>() where T : ControlEventArgs, new()
    {
    ...
    public static void Return<T>(T? value) where T : ControlEventArgs, new()
    {
        ThreadSafety.DebugAssertMainThread();
        if (value is null)
            return;
    ...
    private static class Storage<T> where T : ControlEventArgs, new()
    {
