...
public sealed class ImGuiSolidColorPattern : SpannablePattern
{
    ...
    /// <inheritdoc/>
    protected override PatternRenderPass CreateNewRenderPass() => new ImGuiColorRenderPass(this);

    /// <summary>A state for <see cref="LayeredPattern"/>.</summary>
    private class ImGuiColorRenderPass(ImGuiSolidColorPattern owner) : PatternRenderPass
    {
        public override unsafe void DrawSpannable(SpannableDrawArgs args)
        {
            ...
            args.DrawListPtr.AddRectFilled(lt, rb, color4, owner.Rounding, owner.RoundingFlags);
        }
    }
}
