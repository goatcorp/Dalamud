...
internal static class PluginValidator
{
    ...
    public static IReadOnlyList<IValidationProblem> CheckForProblems(LocalDevPlugin plugin)
    {
        ...
        foreach (var cmd in cmdManager.GetHandlersByWorkingPluginId(plugin.EffectiveWorkingPluginId)
                                      .Where(c => c.CommandInfo.ShowInHelp))
        {
            if (string.IsNullOrEmpty(cmd.CommandInfo.HelpMessage))
                problems.Add(new CommandWithoutHelpTextProblem(cmd.Command));
        }
