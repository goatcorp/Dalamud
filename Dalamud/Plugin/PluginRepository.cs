...
namespace Dalamud.Plugin
{
    internal class PluginRepository
    {
        ...
        private static string PluginMasterUrl => "https://raw.githubusercontent.com/goatcorp/DalamudPlugins/master/pluginmaster.json";
                ...
                foreach (var installed in pluginsDirectory.GetDirectories())
                {
                    try
                    {
                        ...
                        if (!isEnabled && File.Exists(Path.Combine(latest.FullName, ".testing")))
                        {
                            ...
                            if (versions.Any(version => !File.Exists(Path.Combine(version.FullName, ".disabled"))))
                            {
                                isEnabled = true;
                            }
                        }
