...
internal sealed unsafe partial class DalamudAtkTweaks : IServiceType
{
    ...
    private DalamudAtkTweaks(TargetSigScanner targetSigScanner)
    {
        var openSystemMenuAddress = targetSigScanner.ScanText("E8 ?? ?? ?? ?? 32 C0 4C 8B AC 24 ?? ?? ?? ?? 48 8B 8D ?? ?? ?? ??");
        ...
        var atkValueChangeTypeAddress = targetSigScanner.ScanText("E8 ?? ?? ?? ?? 45 84 F6 48 8D 4C 24 ??");
        this.atkValueChangeType = Marshal.GetDelegateForFunctionPointer<AtkValueChangeType>(atkValueChangeTypeAddress);

        var atkValueSetStringAddress = targetSigScanner.ScanText("E8 ?? ?? ?? ?? 41 03 ED");
        this.atkValueSetString = Marshal.GetDelegateForFunctionPointer<AtkValueSetString>(atkValueSetStringAddress);

        var uiModuleRequestMainCommandAddress = targetSigScanner.ScanText("40 53 56 48 81 EC ?? ?? ?? ?? 48 8B 05 ?? ?? ?? ?? 48 33 C4 48 89 84 24 ?? ?? ?? ?? 48 8B 01 8B DA 48 8B F1 FF 90 ?? ?? ?? ??");
        ...
        var atkUnitBaseReceiveGlobalEventAddress = targetSigScanner.ScanText("48 89 5C 24 ?? 48 89 7C 24 ?? 55 41 56 41 57 48 8B EC 48 83 EC 50 44 0F B7 F2");
