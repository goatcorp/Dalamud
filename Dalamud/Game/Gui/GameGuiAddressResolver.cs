...
internal sealed class GameGuiAddressResolver : BaseAddressResolver
{
    ...
    {
        this.SetGlobalBgm = targetSig.ScanText("4C 8B 15 ?? ?? ?? ?? 4D 85 D2 74 58");
        this.HandleItemHover = targetSig.ScanText("E8 ?? ?? ?? ?? 48 8B 5C 24 ?? 48 89 AE ?? ?? ?? ?? 48 89 AE ?? ?? ?? ??");
        this.HandleItemOut = targetSig.ScanText("48 89 5C 24 ?? 57 48 83 EC 20 48 8B FA 48 8B D9 4D");
        this.HandleActionHover = targetSig.ScanText("E8 ?? ?? ?? ?? E9 ?? ?? ?? ?? 83 F8 0F");
        this.HandleActionOut = targetSig.ScanText("48 89 5C 24 ?? 57 48 83 EC 20 48 8B DA 48 8B F9 4D 85 C0 74 1F");
        this.HandleImm = targetSig.ScanText("E8 ?? ?? ?? ?? 84 C0 75 10 48 83 FF 09");
        this.GetMatrixSingleton = targetSig.ScanText("E8 ?? ?? ?? ?? 48 8D 4C 24 ?? 48 89 4c 24 ?? 4C 8D 4D ?? 4C 8D 44 24 ??");
        this.ToggleUiHide = targetSig.ScanText("48 89 5C 24 ?? 48 89 74 24 ?? 57 48 83 EC 20 0F B6 B9 ?? ?? ?? ?? B8 ?? ?? ?? ??");
        this.Utf8StringFromSequence = targetSig.ScanText("48 89 5C 24 ?? 48 89 74 24 ?? 57 48 83 EC 20 48 8D 41 22 66 C7 41 ?? ?? ?? 48 89 01 49 8B D8");
    }
}
