...
internal sealed class ChatGuiAddressResolver : BaseAddressResolver
{
    ...
    protected override void Setup64Bit(TargetSigScanner targetSig)
    {
        ...
        this.PrintMessage = targetSig.ScanText("40 55 53 56 41 54 41 57 48 8D AC 24 ?? ?? ?? ?? 48 81 EC 20 02 00 00 48 8B 05");
        ...
        this.PopulateItemLinkObject = targetSig.ScanText("48 89 5C 24 08 57 48 83 EC 20 80 7A 06 00 48 8B DA 48 8B F9 74 14 48 8B CA E8 32 03 00 00 48 8B C8 E8 ?? ?? ?? FF 8B C8 EB 1D 0F B6 42 14 8B 4A");

        this.InteractableLinkClicked = targetSig.ScanText("E8 ?? ?? ?? ?? E9 ?? ?? ?? ?? 80 BB ?? ?? ?? ?? ?? 0F 85 ?? ?? ?? ?? 80 BB") + 9;
    }
}
